        ...
        try {
            ...
            switch (entry) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    System.out.print("Minimun number of history root ( >=3 ) ? ");
                    int miR = in.nextInt();
                    if (miR >= 3 && miR <= 10) {
                        this.minRoot = miR;
                    } else {
                        System.out.println("ERROR : Minimum number of history root must be >= 3 and <=10  ! ");
                        readEntry(entry);
                    }
                    break;
                case 4:
                    System.out.print("Maximun number of history root ( <=10 ) ? ");
                    int maR = in.nextInt();
                    if (maR >= this.minRoot && maR <= 10) {
                        this.maxRoot = maR;
                    } else {
                        System.out.println("ERROR : Maximum number of history root must be >= minimum and <= 10 ! ");
                        readEntry(entry);
                    }
                    break;
            }
